...
The GLM provides an overarching framework for estimation and testing in a broad class of models. For any GLM, the following apply.
  ...
  - A hypothesis test of $H_0: \beta=0$ versus $H_A: \beta \neq 0$ can be obtained using a z-statistic. In linear regression, an "exact" test (t-test) is available if the response is normally distributed, and for this reason the t-test is the default for linear regression. In GLM's, the z-tests are approximate and require a "big enough" sample size.
...
We can interpret the estimate $\hat{\beta}_1=0.0234$ as the log OR for a one-unit increase in decades: the odds of smoking for a group one decade older is $\exp(0.0234)$ = `r round(exp(0.0234),3)` times the odds of the referent group.  Alternatively, each 10 year increment in age is associated with  `r round(exp(0.0234),3)` times the odds of smoking. Older individuals have higher odds of smoking.  
...
```{r replot, echo = FALSE, warning = FALSE, out.width = "50%"}
ggplot(gatsbin, aes(x = GENDER,
                 fill =CURRENTSMOKE)) +
  geom_bar() + 
  labs(x = 'Gender',
       fill = "Current Smoking") +
...
```{r occbin, echo = FALSE, warning = FALSE, out.width = "50%"}
ggplot(gatsbin, aes(x = OCCUPATION,
                 fill = CURRENTSMOKE)) +
  geom_bar() +
  labs(x = 'OCCUPATION',
       fill = "Current Smoking") +
...
This is an example of **effect measure modification** -- the association between age and smoking depends on gender.
