...
```{r echo=FALSE}
knitr::include_graphics("img/PeanutsMath.png")
```
...
Small p-values are consistent with values of $\bar{X}$ far from the hypothesized mean $\mu_0$
...
Small p-values are consistent with values of $\bar{X}$ much smaller than the hypothesized mean $\mu_0$
...
Small p-values are consistent with values of $\bar{X}$ much larger than the hypothesized mean $\mu_0$
...
Pairing also leads to violation of the independence assumption of many statistical tests, so when our design is paired, we want to use an analysis that reflects the pairing.
...
Note: we are subtracting values after flushing from initial values. If flushing works, will these differences be positive or negative?
...
Do the data provide evidence that flushing for 45 seconds is effective?
...
Do the data provide evidence that flushing for 2 minutes is effective?
...
.pull-right[
An alternative would be to obtain paired or matched samples and test the equality of means $\mu_1$ and $\mu_2$.  Matching could be by person (e.g., before and after measures) or could be a pair of individuals who belong together in another way (e.g., same date of birth in same hospital, life partners).
