...
along with the type of each column. Try this with the `BMI_long` data, available on Sakai under Resources in the Data folder.
...
R recently introduced an alternative pipe, which is `|>` instead of `%>%`. Feel free to use either one. 
...
summarize to calculate summary statistics by group. Here we find the proportion of means that are in the normal range for each region. If you don't `ungroup()` after you use it, there may be unforeseen consequences down the road, so this is good practice.
...
BMI_long %>%
  ...
  mutate(normal_range = bmiclass == "Normal Range") %>%
  group_by(Region) %>%
  summarize(fracnormal = mean(normal_range,na.rm=TRUE)) %>% ungroup()
```
]
.panel[.panel-name[Output]
