...
We start by combining data from two Excel files. The first file provides population data from 1960 to 2021 for World Bank regions and countries, and the second file contains the fertility rate for the same entities from 1960-2020. The code is suppressed here, but you can view it on the class organization's GitHub page (under website, static, slides, week-02, lab-02-data-viz.Rmd) if you want to see it.  That code creates the file we'll use in analysis, called `worldbankcountries`.

```{r readdata, echo=FALSE, message=FALSE, warning=FALSE}
library(readxl)

#NOTE:  later we'll learn more about how to do this data wrangling. For now, you can ignore this -- just note that we end up with a dataset called worldbankcountries
...
1. Make a histogram of the 2020 population of the countries in the data set `worldbankcountries`. If you need a refresher on adding ggplot code, refer to week 1 slides. Set the bin width at 100,000,000. Label your axes and give the plot a title. Make sure the axis ticks on the x-axis are not in scientific notation using the scale option *scale_x_continuous(labels = label_comma()) *. Does it appear that there are any outliers (values that are far away from the main body of the data)? If so, what are they (Hint: use the data viewer)?  Does the distribution appear to be symmetric or skewed?


```{r ex1, eval=FALSE, warning=FALSE}
#NOTE: you need to delete "eval=FALSE" if you want to run the code!
worldbankcountries %>%
  filter(Year == 2020) %>%
  # you do the rest!
...
```{r ex2, message=FALSE, warning=FALSE, eval=FALSE}
...
3. Filter the data to include China, Jordan, the Republic of Korea, Somalia, the US, and one other country of interest to you. You might want to save the filtered data to a separate data set, as we'll work with them in subsequent exercises (or you can just repeat the code). To store something to a new variable, at the end of your code you can add ‘-> new_variable_name’ where the arrow indicates assignment and the new_variable _name is where the new dataset will be stored (and you can refer to it in your code moving forward). Generate a scatter plot of `Year` (x-axis) by fertility rate on the y-axis, with points colored by `Country Name` and with size mapped to `Population`. Please label your axes and legend and give the plot a title. 
...
```{r ex3, message=FALSE, warning=FALSE, eval=FALSE}
worldbankcountries %>%
  filter(`Country Name` %in% c("China", "Jordan", #you finish the list and the code)) 
                               %>%
```
...
```{r ex4}
#insert code here
#note: you may wish to show the years on the x-axis at an angle, e.g incorporating code to ggplot like   
...
5. How has the distribution of fertility rates of countries changed in the last 60 years? Create faceted or side-by-side boxplots for the full data (e.g., stop filtering to just your six countries) for the years 1960, 1980, 2000, and 2020. Briefly comment on what you notice.

```{r ex5, warning=FALSE, message=FALSE, eval=FALSE}
worldbankcountries %>%
  filter(Year %in% c(1960,1980,#you finish it)) 
                     %>%
...
6. Reproduce the plot below. Write a short paragraph interpreting the graphs, describing how the fertility rate has changed over time in these countries and drawing comparisons across countries where appropriate. HINT: the Hex color for Duke blue is #00539B.
...
knitr::include_graphics("exercise6.png")
...
- Overall:  8 pts
  ...
  - Overall includes the number of commits made (need at least 3 informative commits), naming chunks, updating the name on the assignment to your name, following tidyverse style (see: https://style.tidyverse.org/), and in general producing a nicely formatted, neat report. 