...
worldbankcountries %>%
  ...
  labs(x = "Population (log scale)",
       y = "Number of Countries",
       title = "2020 Population (log scale)")
...
worldbankcountries %>%
  filter(`Country Name` %in% c("China", "Jordan", "Korea, Rep.", "Somalia", "United States", "Ireland")) %>%
  ggplot(aes(
    x = Year,
    y = `Fertility Rate`,
    color = `Country Name`,
...
worldbankcountries %>%
  ...
  ggplot(aes(x = as.factor(Year), y= `Fertility Rate`)) +
  geom_boxplot() +
  labs(
    title = "Country-Level Fertility Rates",
    subtitle = "1960, 1980, 2000, and 2020",
    x = "Year"
  )
