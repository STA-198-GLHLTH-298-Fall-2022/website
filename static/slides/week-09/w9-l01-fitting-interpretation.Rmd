...
The *correlation coefficient* describes the linear relationship between $x$ and $y$.

```{r corr}
cor(mercury$lhairHg,mercury$assets_sc,use="complete.obs")
```

The correlation coefficient is on a scale from -1 to 1 and reflects the linear association between $x$ and $y$. When it is positive, larger values of $x$ are associated with larger values of $y$, and when it is negative, larger values of $x$ are associated with smaller values of $y$. When it is zero, we don't see a linear relationship between $x$ and $y$. 
