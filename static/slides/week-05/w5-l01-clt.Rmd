...
- Use bootstrapping to create confidence intervals for any quantity of interest
...
The *central limit theorem* says that for any distribution with a well-defined mean and variance, the distribution of *means* for a sample of *independent* observations of size $n$ is approximately normal. This result has strong implications in many areas of statistics, including in construction of confidence intervals and in hypothesis testing.
...
In a 2021 seminar on the COVID epidemic in India, Professor Bhramar Mukherjee noted that 67% of the population there had been infected.  Define the variable $X$ to take value 1 if a randomly sampled Indian resident has been infected, and let it be 0 otherwise.
...
The *Central Limit Theorem* says that for a population with mean $\mu$ and standard deviation $\sigma$, the important three properties of the distribution of sample averages $\bar{x}$ taken over independent observations hold:
...
- test for whether a population parameter takes a prespecified value by evaluating how likely it is to obtain the
  observed sample statistic, or one even further away from the prespecified value, if the population parameter did indeed take the value hypothesized, as 
...
point estimate $\pm$ margin of error  (m)
...
This gives us a *95% confidence interval* for $\mu$ of $\left(\bar{X}-1.96\frac{\sigma}{\sqrt{n}}, \bar{X}+1.96\frac{\sigma}{\sqrt{n}} \right)$. In terms of $\pi$, this is $\left(\bar{X}-1.96\frac{\sqrt{\pi(1-\pi)}}{\sqrt{n}}, \bar{X}+1.96\frac{\sqrt{\pi(1-\pi)}}{\sqrt{n}} \right)$
...
- As the sample size gets bigger, we get a better estimate of $\sigma$
