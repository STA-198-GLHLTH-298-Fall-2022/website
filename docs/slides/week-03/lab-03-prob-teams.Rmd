...
The Repository for Global.health is a data science initiative to enable rapid sharing of trusted and open public health data to advance the response to infectious diseases. In this lab we use their monkeypox data repository, which is updated daily. We start by visualizing these data.
<!-- BETTER DATA WOO HOO
mpx=read.csv("https://raw.githubusercontent.com/owid/monkeypox/main/owid-monkeypox-data.csv")
--> 

```{r mapmonkeypox, echo=FALSE, warning=FALSE, comment=FALSE}
library(tidyverse)
library(maps)
# https://www.who.int/emergencies/disease-outbreak-news/item/2022-DON385 lists the
# endemic countries with monkeypox for reference
# Read worldwide case data ref: https://www.thelancet.com/journals/laninf/article/PIIS1473-3099(22)00359-0/fulltext

#time series not per million
#case_series <- read.csv("https://raw.githubusercontent.com/globaldothealth/monkeypox/main/timeseries-country-confirmed.csv")

#this data source standardizes to new and total cases per million population
case_series <-read.csv("https://raw.githubusercontent.com/owid/monkeypox/main/owid-monkeypox-data.csv")

world_map <- map_data("world")
# fix var names, drop "World" in input region
plot_case_map <- function(date, xlim, ylim) {
  # Pre-process case and map data
  case_map <- case_series[which(case_series$date == date), c(1, 7)] #location and total cases/mil
...
# Plot world case map as of Aug 22nd 2022:
plot_case_map("2022-08-22", c(-180, 180), c(-55, 90))
```
