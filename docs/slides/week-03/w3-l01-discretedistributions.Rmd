...
When we learned how to find probabilities using basic principles, we focused on one particular outcome or event, like the probability of a randomly selected person being a smoker. We now explore the bigger picture by exploring all possible values of a discrete random variable, along with their associated probabilities. This list of possible values and probabilities is called the *probability distribution* of the random variable.
...
Let's use R to take a look at the distribution of the number of recent e-cig smokers in groups of varying size.  We can actually run an experiment in R using the CDC estimate of the percentage of high schoolers who have smoked e-cigarettes in the past 30 days, 19.6%. Let's draw a single random sample of 1000 high schoolers with smoking probability 0.196. Our random variable $X$ is now the number of smokers in this big sample.
...
```{r plotbinomdatanoecho,echo=FALSE,out.width="40%"}
ggplot(binomdata, aes(x = X)) + 
  geom_histogram() +
  labs(x = "Number of Smokers",
       title = "Binomial(1000,0.196) Distribution")
...
Suppose you conduct a survey of 1000 Durham, NC high school students and find that 300 of them have smoked e-cigarettes in the past 30 days. If we estimate the probability of e-cig use based on the proportion who reported smoking in this survey, what is our estimate? Based on these data, do you think e-cigarette smoking rates in Durham high schools are equivalent to those reported by the CDC?

---

Now suppose that because of the time and money involved, you could only survey 10 Durham high school students. 

```{r smallbinom, eval=FALSE}
binomdatasmall = data.frame(X = rbinom(1000000, 10, 0.196))

ggplot(binomdatasmall, aes(x = X)) + 
  geom_histogram() +
  labs(x = "Number of Smokers",
       title = "Binomial(10,0.196) Distribution")
...
The plot shows the binomial distribution with the same probability of smoking (19.6%) but with samples of $n=10$ students. (Question: why don't the bars touch here, when they touched before?)

```{r smallnoecho, echo=FALSE}
binomdatasmall2 = data.frame(X = rbinom(1000000, 10, 0.196))
ggplot(binomdatasmall2, aes(x = X)) + 
  geom_histogram() +
  labs(x = "Number of Smokers",
       title = "Binomial(10,0.196) Distribution")
```
...
Suppose in your smaller survey, you found 3 students had smoked e-cigs in the past 30 days. What is our estimated probability of smoking based on Durham data now?  What can you say in this small study about how Durham's e-cig smoking rates compare to national levels reported by CDC?
...
Suppose we prefer a singleton birth, twins are ok (though not without increased risk), and the risk of higher-order births should be kept to a minimum.  Assume the probability of each embryo surviving is independent of the others.
...
- We can use the binomial distribution to find $P(X \geq 1)$ and $P(X>2)$ for $n=1,2,3,4,5$
...
Binomial distribution: $P(X=x)=\begin{pmatrix} n \\ x \end{pmatrix}0.35^x(1-0.35)^{n-x}$


More generally,

$$\begin{aligned}
P(X \geq 1) & =1-P(X=0)=1-\begin{pmatrix}n \\ 0 \end{pmatrix} 0.35^0 (0.65)^{n-0}=1-0.65^n \\
...
- $P(X\geq 1)=1-0.65^n$
- $P(X > 2)=1 - 0.65^n - n(0.35)(0.65)^{n-1} - \frac{n(n-1)}{2}0.35^2(0.65)^{n-2}$

If $n=5$, then we have

- $P(X\geq 1)=1-0.65^5=0.88$ (good chance of a baby!)
- $P(X > 2)=1 - 0.65^5 - 5(0.35)(0.65)^{5-1} - \frac{5(4)}{2}0.35^2(0.65)^{5-2}=0.24$ (good chance of lots o' babies!)
...
For the last row of the table on the prior slide, $n=5$ and $\pi=0.35$. The probability of at least one baby, $P(X \geq 1)=1-P(X=0)$ can be calculated using in R the code *1-dbinom(0,5,0.35)*, which yields the value `r 1-dbinom(0,5,0.35)`.
