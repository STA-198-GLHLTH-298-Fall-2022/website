...
How would the plot look if males and females had the same life expectancy on average?
...
## Accommodating more than 6 shapes

.pull-left[
```{r shape-region-fixed, fig.show = "hide", warning = FALSE}
ggplot(lifeexpwide2019,
       aes(x = Female, 
           y = Male,
           shape = worldbankregion)) +
...
.pull-right[
```{r ref.label = "shape-region-fixed", echo = FALSE, warning = FALSE, out.width = "100%"}
```
]
...
<span style="color: red;">W</span>here <span style="color: red;">T</span>o <span style="color: red;">F</span>ind all this code?

If you're starting to freak out about all the code, don't worry. We'll start off by giving you code chunks you just need to modify in order to create similar plots to those used in class or lab. As the course progresses, you'll learn more of the *why* of the coding.

In addition, **Professor Doctor Google** is your friend (and mine!). I often consult them when R is being persnickety.
...
Maybe ask Dr. Google how to get rid of the scientific notation...
...
.pull-left[
```{r size3, fig.show = "hide", warning = FALSE}
library(scales)
ggplot(lifeexpwide2019,
       aes(x = Female, 
           y = Male,
           ...
           size = pop)) +
  scale_size(labels=comma) + 
  labs(col='Region',size='Population') + #<<
  geom_point() 
```
]
.pull-right[
```{r ref.label = "size3", echo = FALSE, warning = FALSE, out.width = "100%"}
```
]
...
lifeexpwide %>%
  filter(year %in% c(1999, 2009,2019),worldbankregion %in% c("East Asia and Pacific","Latin America and Caribbean","Sub-Saharan Africa")) %>%
    ggplot(aes(x = Female,
               y = Male)) + 
  geom_point() +
...
.panelset[
.panel[.panel-name[Plot]
```{r ref.label = "facet2", echo = FALSE, warning = FALSE, out.width = "70%"}
```
]
.panel[.panel-name[Code]

```{r facet2, fig.show = "hide"}
region.labs <- c("E. Asia & Pacific","Europe & Central Asia","Latin Amer. & Caribbean","Middle East & N. Africa","N. Amer.","S. Asia","Sub-Saharan Africa")
names(region.labs) <- c("East Asia and Pacific","Europe and Central Asia","Latin America and Caribbean","Middle East and North Africa","North America","South Asia","Sub-Saharan Africa")
...
lifeexpwide %>%
  filter(year %in% c(1999, 2009,2019),worldbankregion %in% c("East Asia and Pacific","Latin America and Caribbean","Sub-Saharan Africa")) %>%
    ggplot(aes(x = Female,
               y = Male)) + 
  geom_point() +
  facet_grid(year~worldbankregion, labeller = labeller(worldbankregion = region.labs)) #<<
```
]
]
...
lifeexpwide %>%
  filter(year %in% c(1999, 2009,2019),worldbankregion %in% c("East Asia and Pacific","Latin America and Caribbean","Sub-Saharan Africa")) %>%
    ggplot(aes(x = Female,
               y = Male)) + 
  geom_point() +
  facet_wrap(~worldbankregion,labeller = labeller(worldbankregion = region.labs))
```
...
lifeexpwide %>%
  filter(year %in% c(1999, 2009,2019),worldbankregion %in% c("East Asia and Pacific","Latin America and Caribbean","Sub-Saharan Africa")) %>%
    ggplot(aes(x = Female,
               y = Male)) + 
  geom_point() +
  facet_grid(.~worldbankregion,labeller = labeller(worldbankregion = region.labs))
```
