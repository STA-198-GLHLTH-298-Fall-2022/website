...
author: "[Course Website](https://STA-198-GLHLTH-298-Fall-2022.github.io/website/)"
...
For example, instead of the **marginal** probability of dying of COVID (regardless of vaccination status), we may wish to know the probability that someone will die of COVID given that they have been vaccinated, or the probability someone will die of COVID given that they have not been vaccinated. 

These are examples of **conditional probability**. The conditional probability that someone had a preterm birth (B) given that they smoked (C) is denoted $P(B|C)$, which we say as "probability of B given C."

---

## Simple Example

Suppose we have a small data set containing 3 female non-smokers, 1 female smoker, 4 non-female non-smokers, and 4 non-female smokers.
...
We may be interested in the conditional probability someone is a smoker given they identify as female, which we can estimate as $\frac{1}{3+1}$ -- we just change the denominator to correspond to our smaller population of interest.
...
A=preterm birth and B=Non-Hispanic Hawaiian or Pacific Islander. Calculate the following probabilities for a randomly-selected person in this cohort.

- **Marginal probability** of preterm birth, $P(A)$

- **Joint probability** of preterm birth and Non-Hispanic Native Hawaiian or Pacific Islander status, $P(A \cap B)$

- **Conditional probability** of preterm birth given Non-Hispanic Native Hawaiian or Pacific Islander status, $P(A \mid B)$

How might we use conditional probabilities to explore potential racial or ethnic disparities in preterm birth risk in these data?
...
**Check it out**: Are preterm birth and non-Hispanic Native Hawaiian or Pacific Islander status independent in our cohort?
...
For more practice, go back to the birth data example. 

1. Calculate the percentage of each racial/ethnic group in the population provided. We can view this as the **marginal probability** or distribution of each race/ethnicity in the population of people giving birth in the US in 2020.

2. Given that someone is a member of a given race/ethnicity category, calculate the **conditional probability** that the person has a preterm birth, in turn. This information could be useful in helping to target policies and outreach campaigns to reduce preterm births.
