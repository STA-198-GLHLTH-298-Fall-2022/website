...
The **risk ratio (RR)** compares the risk of an event happening in one group compared to another. They are often used in forward-looking (prospective) studies and in randomized trials. A RR=1 means there is no difference between two groups in terms of their risk of the outcome. 

We calculate the RR of mortality as the ratio of conditional probabilities of death for the two serotypes.

---

.panelset[
.panel[.panel-name[Output]
```{r ref.label = "RR", echo = FALSE, warning = FALSE}
```
]
.panel[.panel-name[Code and Interpretation]
```{r RR, results="hide"}
pneu3 <- pneu2 %>% 
  group_by(Serotype, Survived) %>%
  summarize(n=n()) %>% 
  pivot_wider(names_from = Survived,
              values_from = n)
pneu3 <- as.data.frame(pneu3)

library(epitools)
pneu3 <- table(pneu2)

riskratio(pneu3, rev="both", method="wald")
...
Here, we estimate $\widehat{RR}=\frac{9/106}{10/34}=0.29$, 95% CI=(0.13, 0.65), indicating that patients with serotype 20 have just 0.29 times the risk of dying as patients with serotype 31.
